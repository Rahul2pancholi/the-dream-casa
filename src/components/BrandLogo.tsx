...
interface BrandLogoProps {
  /** "dark" for light cream backgrounds, "light" for dark navy/charcoal backgrounds */
  variant?: "dark" | "light";
  className?: string;
}
...
export default function BrandLogo({ variant = "dark", className = "" }: BrandLogoProps) {
  ...
  const archColor = isLight ? "#ffffff" : "#a46f47";
  const windowColor = isLight ? "#b8933f" : "#a46f47";
  const textColor = isLight ? "#ffffff" : "#0F1A24";
  const subtextColor = isLight ? "#b8933f" : "#a46f47";
  const separatorColor = isLight ? "rgba(184,147,63,0.5)" : "#E8E2D8";
